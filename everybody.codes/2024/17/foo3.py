...
for i, star in enumerate(stars):
    for j, other in enumerate(stars):
        if i != j:
            dists[(i, j)] = manhattan(star, other)

clusters = set()

seen = set()
def cluster_from(star):
    cluster = set([star])
    q = deque([star])
...
def size(cluster):
    ...
    for i, star in enumerate(cluster):
        for j, other in enumerate(cluster):
            if i != j:
                g[i][j] = dists[(star, other)]

    cost = kruskal_minimum_spanning_tree(g)
    return cost + len(cluster)

sizes = list(map(size, clusters))
print(prod(list(sorted(sizes, reverse=True))[0:3]))
