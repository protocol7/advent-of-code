...
def foo(cur, rem, closed):
    ...
    # try each child
    for c, dist in d[cur]:
        ps = max(ps, foo(c, rem-dist, closed))

    return ps

# part 1
print(foo(aa, 30, reduce(bit_set, closed, 0)))

# part 2
print("this takes a while...")
ps = []
for i in range(len(closed) + 1):
    for mine in combinations(closed, i):
        elephants = closed - set(mine)

        m = foo(aa, 26, reduce(bit_set, mine, 0))
        e = foo(aa, 26, reduce(bit_set, elephants, 0))
